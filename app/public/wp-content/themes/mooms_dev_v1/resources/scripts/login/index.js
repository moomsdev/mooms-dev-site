...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (userLogin) userLogin.setAttribute('placeholder', 'Username or Email Address');
...
async function handleGoogleLogin(e) {
    ...
    try {
        ...
        if (data.success && data.data.redirect_url) {
            ...
            showLoginError((data.data && data.data.message) || 'Có lỗi xảy ra khi đăng nhập Google');
...
function showLoginSuccess(notification) {
    ...
    setTimeout(() => { if (successContainer.parentNode) successContainer.parentNode.removeChild(successContainer); }, 3000);
}
