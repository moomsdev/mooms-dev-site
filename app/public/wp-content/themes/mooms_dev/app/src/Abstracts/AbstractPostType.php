...
abstract class AbstractPostType
{
	...
	public function __construct()
	{
		...
		//     Container::make('post_meta', __('Cài đặt', 'mms'))
		...
		//         ->add_fields([
		//             Field::make('checkbox', 'is_feature', __('Nổi bật', 'mms'))->set_default_value(false),
		//         ]);
		// });
	...
	protected function getLabels()
	{
		return [
			...
			'add_new'               => __('Add new', 'mms'),
			'add_new_item'          => __('Add new post ' . $this->singularName, 'mms'),
			'edit_item'             => __('Edit post', 'mms'),
			'new_item'              => __('New post', 'mms'),
			'view_item'             => __('View post', 'mms'),
			'search_items'          => __('Search', 'mms'),
			'not_found'             => __('Not found', 'mms'),
			'not_found_in_trash'    => __('Not found in trash', 'mms'),
			'parent_item_colon'     => __('Parent:', 'mms'),
			'all_items'             => __('All posts', 'mms'),
			'archives'              => __($this->pluralName, 'mms'),
			'insert_into_item'      => __('Insert posts', 'mms'),
			'uploaded_to_this_item' => __('Uploaded to this posts', 'mms'),
			'featured_image'        => __('Featured image', 'mms'),
			'set_featured_image'    => __('Set featured image', 'mms'),
			'remove_featured_image' => __('Remove featured image', 'mms'),
			'use_featured_image'    => __('Use featured image', 'mms'),
...
?>
	...
	public function editAdminColumn($columns): array
	{
		...
		//        $columns     = insertArrayAtPosition($columns, ['is_feature' => __('Nổi bật', 'mms')], $totalColumn - 1);
	...
	public function addMetaBoxBanDo()
	{
		add_action('add_meta_boxes', function () {
			add_meta_box('mms_meta_map_box', __('Vị trí trên bản đồ', 'mms'), [$this, 'hienThiMetaBoxBanDo'], $this->post_type, 'normal', 'high');
		});
	...
	{
		Container::make('post_meta', __('Thông tin vị trí điểm đầu - cuối', 'mms'))
			->set_context('carbon_fields_after_title') // normal, advanced, side or carbon_fields_after_title
			->set_priority('default') // high, core, default or low
			->where('post_type', 'IN', [$this->post_type])
			->add_fields([
				Field::make('text', 'dia_chi_dau', __('Địa chỉ (điểm đầu)', 'mms'))
					->set_width(100),
				Field::make('select', 'tinh_dau', __('Tỉnh / Thành phố (điểm đầu)', 'mms'))
					...
					->set_width(33),
				Field::make('select', 'quan_huyen_dau', __('Quận / Huyện (điểm đầu)', 'mms'))
					->add_options(['---- Chọn quận / huyện ----'])
					->set_width(33),
				Field::make('select', 'phuong_xa_dau', __('Phường / Xã (điểm đầu)', 'mms'))
					->add_options(['---- Chọn phường / xã ----'])
					->set_width(34),
				Field::make('text', 'toa_do_x_dau', __('Tọa độ X (điểm đầu)', 'mms'))
					->set_width(50),
				Field::make('text', 'toa_do_y_dau', __('Tọa độ Y (điểm đầu)', 'mms'))
					->set_width(50),

				Field::make('text', 'dia_chi_cuoi', __('Địa chỉ (điểm cuối)', 'mms'))
					->set_width(100),
				Field::make('select', 'tinh_cuoi', __('Tỉnh / Thành phố (điểm cuối)', 'mms'))
					->add_options(function () {
						$posts   = get_posts([
							'post_type'      => 'dia-phan-tinh',
							'posts_per_page' => -1,
					...
					->set_width(33),
				Field::make('select', 'quan_huyen_cuoi', __('Quận / Huyện (điểm cuối)', 'mms'))
					->add_options(['---- Chọn quận / huyện ----'])
					->set_width(33),
				Field::make('select', 'phuong_xa_cuoi', __('Phường / Xã (điểm cuối)', 'mms'))
					->add_options(['---- Chọn phường / xã ----'])
					->set_width(34),
				Field::make('text', 'toa_do_x_cuoi', __('Tọa độ X (điểm cuối)', 'mms'))
					->set_width(50),
				Field::make('text', 'toa_do_y_cuoi', __('Tọa độ Y (điểm cuối)', 'mms'))
					->set_width(50),
			]);
	...
	{
		Container::make('post_meta', __('Thông tin vị trí', 'mms'))
			->set_context('carbon_fields_after_title') // normal, advanced, side or carbon_fields_after_title
			->set_priority('default') // high, core, default or low
			->where('post_type', 'IN', [$this->post_type])
			->add_fields([
				Field::make('text', 'dia_chi', __('Địa chỉ', 'mms'))
					->set_width(100),
				Field::make('select', 'tinh', __('Tỉnh / Thành phố', 'mms'))
					->add_options(function () {
						$posts   = get_posts([
							'post_type'      => 'dia-phan-tinh',
							'posts_per_page' => -1,
					...
					->set_width(33),
				Field::make('select', 'quan_huyen', __('Quận / Huyện', 'mms'))
					->add_options(['---- Chọn quận / huyện ----'])
					->set_width(33),
				Field::make('select', 'phuong_xa', __('Phường / Xã', 'mms'))
					->add_options(['---- Chọn phường / xã ----'])
					->set_width(34),
				Field::make('text', 'toa_do_x', __('Tọa độ X', 'mms'))
					->set_width(50),
				Field::make('text', 'toa_do_y', __('Tọa độ Y', 'mms'))
					->set_width(50),
			]);
